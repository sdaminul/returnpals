...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="seco-title">{{ $inTransitCount }} package{{ $inTransitCount === 1 ? '' : 's' }} in transit</div>
        </div>
...
@endphp
                                    @foreach($package->items as $item)
                                        {{ $item->product_name }} <span class="mx-1">x{{ $item->quantity }}</span>
                                        @php
...
<span class="badge {{ $conditionClass }} py-1 px-2 fs-12">
    {{ $item->condition }}
</span>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="badge {{ $statusClass }} py-1 px-2 fs-12">{{ $package->status }}</span>
                                </td>
                                <td>{{ $package->created_at?->format('n/j/Y') }}</td>
                                <td>{{ $package->notes }}</td>
                                <td>
                                    <a href="#" class="edit-package"
                                       data-bs-toggle="modal" data-bs-target="#editPackage"
                                       data-id="{{ $package->id }}"
                                       data-reference="{{ $package->reference }}"
                                       data-notes="{{ e($package->notes ?? '') }}"
                                       data-items="{{ json_encode($package->items_json) }}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="bg-light rounded mb-3">
                        <div class="p-3">
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <a href="{{ asset('dashboard/assets/ReturnPal_Package_Template.xlsx') }}" class="btn btn-success"><i class="ri-download-2-line me-1 fs-18"></i>Download</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('dashboard.packages.bulk.commit') }}" id="bulkCommitForm" class="d-none">
                        @csrf
...
(function(){
    ...
    document.querySelectorAll('.edit-package').forEach(link => {
        link.addEventListener('click', function(){
            ...
            form.action = `{{ url('/dashboard/packages') }}/${id}`;

            const deleteForm = document.getElementById('deletePackageForm');
            deleteForm.action = `{{ url('/dashboard/packages') }}/${id}`;
    ...
    if(bulkPreviewBtn){
        bulkPreviewBtn.addEventListener('click', async function(){
            ...
            try{
                const res = await fetch('{{ route('dashboard.packages.bulk.preview') }}', {
                    method: 'POST',
                    body: fd,
